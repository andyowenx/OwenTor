			...
			const char *id_digest, channel_tls_t *chan))
{
	...
	//-----my code start-----
	//-----if link to my machine-----
	/*
	if (strcmp("140.123.103.190",inet_ntoa(addr.addr.in_addr))==0 ||
		strcmp("140.123.103.191",inet_ntoa(addr.addr.in_addr))==0 ||
		strcmp("140.123.103.192",inet_ntoa(addr.addr.in_addr))==0 ){
		//-----check the connection number of link-----
		//-----do something-----
		//-----if need a new connection , change the port and create it, else return the states of connections-----
	}
	*/
	//-----my code end-----
